...
def spawn_ball(direction):
    ...
    if direction == RIGHT:
        ball_vel = [random.randrange(120, 240) / 60, random.choice([-1, 1]) * random.randrange(60, 180) / 60]
    if direction == LEFT:
        ball_vel = [-random.randrange(120, 240) / 60, random.choice([-1, 1]) * random.randrange(60, 180) / 60]
...
def draw(canvas):
    ...
    # determine whether paddle or side walls and ball collide    
    ...
    # collision with top or bootom walls
    ...
    # draw scores
    canvas.draw_text(str(score1), [WIDTH / 3, HEIGHT / 2 + 20], 64, "White")
    canvas.draw_text(str(score2), [WIDTH * 2 / 3, HEIGHT / 2 + 20], 64, "White")
...
frame.add_button('Restart', button_start_handler, 100)
